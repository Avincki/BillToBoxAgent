...
## 2026-06-20 — WORKPLAN task 22: systemd units

The timer/oneshot/dashboard trio, mirroring HEC's field conventions (CONVENTIONS.md §10/§13-B)
exactly — dedicated `User=billtobox`/`Group=`, `WorkingDirectory=/opt/billtobox/BillToBoxAgent`,
**inline `Environment=BTB_CONFIG=…`** (never EnvironmentFile), `TimeoutStopSec=15`, and the full
hardening block (`NoNewPrivileges`/`PrivateTmp`/`ProtectSystem=strict`/`ProtectHome`/
`ReadWritePaths=…app dir`). Files under `deploy/systemd/`:

- `billtobox-agent.service` — `Type=oneshot`, `ExecStart=…/.venv/bin/python -m
  billtobox_agent.worker`; **no [Install]** (driven by the timer).
- `billtobox-agent.timer` — `OnCalendar=*-*-* 07:00:00` (decisions.md #8) with a documented
  `systemctl edit` drop-in for overriding, `Persistent=true`, `WantedBy=timers.target`.
- `billtobox-dashboard.service` — `Type=simple`, long-lived, `ExecStart=…/python main.py`,
  `Restart=on-failure`/`RestartSec=5`, `WantedBy=multi-user.target`. The dashboard port is **not**
  hardcoded — main.py reads `web.port` (9003) from config.

`scripts/install-units.sh` symlinks the three units into `/etc/systemd/system/`, `daemon-reload`s,
and prints the enable/verify commands (requires root; symlinks so `git pull` keeps them current).

`systemd-analyze verify` can only run on the Pi (task 25), so added `tests/unit/test_systemd_units.py`
(4 tests) to keep the units from drifting from the locked conventions in CI on any platform —
asserting Type/User/paths/inline-config/hardening per unit, the timer schedule + install target, no
[Install] on the worker, and that install-units.sh links all three + reloads. Toolchain green under
the 3.12 venv: ruff ✓, ruff-format ✓, black ✓, mypy ✓ (48 files), pytest ✓ (177, up from 173).
